...
\drafttodo{Items (i)--(ii) now have complete binary proofs in
Sections~\ref{sec:operators}--\ref{sec:algorithm}, but still require an
independent proof audit.  Item (iii) remains open until the cost-model proof is
complete.  Retain items (iv)--(v) only to the extent supported by
...
\]
Finally, $N^d=0$ for $d\ge m$.  Since the distances $\Delta_t$ are distinct,
the surviving shifts in the displayed sum cannot cancel.
\end{proof}
...
counts.  A feedback shift by $d<m$ touches
...
\[
  ...
  \qquad(0\le k<r).
\]
The scheduled shifted-word contributions of generalized Suwako reduction are
\begin{align}
  B_{\rm fb}
...
$n$-word final assembly pass, in addition to the $B_{\rm fb}+B_V$ shifted
contributions.
\end{proposition}
\begin{proof}
...
words.  The extraction and assembly statements follow from traversing the
$m$-bit input and output states once.
\end{proof}
...
schedule but can execute up to $\lceil m/\Dmin\rceil$ dependent feedback
iterations; L\'opez--Dahab has $\Theta(n(1+s))$ shift/scatter work under its
...
\midrule
Feedback decomposition and correctness & proof drafted; audit open & 20,000 random
GF(2) trials and 299,592 exhaustive binary modulus/input pairs\\
Frobenius factors and inverse & proof drafted; audit open & independent agreement over
GF(2), $\mathbb F_4$, dual numbers, and an $\mathbb F_3$ sign/radix check\\
Exact feedback-stage count & proof drafted; audit open & native schedule checks at all
72 tested $(m,\Dmin)$ coordinates\\
Scheduled work $\Wfb$ and coarse bound & proof drafted; audit open & all 262,125
nonempty supports for $2\le m\le18$ pass; fixed-$m$ runtime correlations are
$0.987$--$0.993$\\
Random-support laws & proof drafted; audit open & exact geometry for 10,623 frozen
